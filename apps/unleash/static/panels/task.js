...
import { prettyRobotId } from "/app/panels/classes.js";

export function initTasks(root) {
  const tasks = new Map(); // task_id -> {winner, score, ts_ms, pretty}
  ...
  function render() {
    ...
    if (tasks.size === 0) {
      ...
      e.className = "empty";
      e.textContent = "No tasks announced yet. Waiting for the bootnode to post the initial auction pool…";
      root.appendChild(e);
      return;
    }
      ...
      card.innerHTML = `
        <div class="task-header">
          <span>
            ...
            <span class="task-pretty">${escape(t.pretty || "")}</span>
          </span>
          <span class="task-status ${status}">${status}</span>
        </div>
        <div class="task-body">
          <div><em>winner</em> <b>${t.winner ? escape(prettyRobotId(t.winner)) : "—"}</b></div>
  ...
  return {
    onWinner: (d) => {
      ...
      t.score = d.bid_score;
      t.ts_ms = d.ts_ms;
      if (d.task_pretty) t.pretty = d.task_pretty;
